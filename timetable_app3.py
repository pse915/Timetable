...
# ==========================================================================================
# 0. 기본 설정 / 상수 / 유틸리티 & Custom CSS (표 가독성 강화)
# ==========================================================================================
st.set_page_config(page_title="시간표·결보강 관리", page_icon="📘", layout="wide")

# 표 구분선 및 가독성 향상을 위한 Custom CSS 주입
st.markdown("""
<style>
    /* Streamlit DataFrame 및 Table 테두리/격자선 강화 */
...
MAX_HISTORY = 12  # 메모리 과다 사용 방지를 위해 히스토리 최대 개수 경량화
...
# ==========================================================================================
# 1. 히스토리 (Undo / Redo) 및 메모리 누수 방지 최적화
# ==========================================================================================
def push_history(action_name: str = "작업"):
    ...
    # deep=True 복사로 메모리 포인터 참조 엉킴 및 메모리 누수 방지
    snapshot = {
        "action": action_name,
...
# ==========================================================================================
# 3. 핵심 로직 & 성능 최적화 (맞교환 실시간 동적 반영 로직)
# ==========================================================================================
def get_effective_timetable_for_date(on_date: str) -> pd.DataFrame:
    """맞교환(swaps) 내역을 정확히 반영하여 특정 날짜의 최종 변경된 시간표를 동적으로 생성"""
...
with tabs[3]:
    ...
    if day not in DAYS:
        ...
        # [수정 핵심] 원본 시간표 대신 해당 날짜의 맞교환 반영 시간표(eff_tt)를 조회하여 교체된 수업도 즉시 표시
...
with tabs[8]:
    ...
    if not teacher_list:
        ...
        st.caption(f"주간: {week_dates[0].strftime('%Y-%m-%d')} ~ {week_dates[4].strftime('%Y-%m-%d')} (결강·보강·맞교환 실시간 반영)")
        ...
        with col_ta:
            ...
            # [수정] 맞교환 내역이 반영된 실시간 시간표에서 선택하도록 개선
...
with tabs[10]:
    ...
    if abs_day not in DAYS:
        ...
        if target_lessons.empty:
            ...
            st.markdown(f"#### 📌 {target_teacher} 교사의 {abs_day}요일 수업 목록 ({len(target_lessons)}시간 - 실시간 변경 반영됨)")
