...
==========================================================================================
 ...
 2026 서라벌여자중학교용 – Google Sheets 연동 성능 최적화, 고대비/색맹 접근성 및 반응형 강화
==========================================================================================
"""
...
# ==========================================================================================
st.set_page_config(
    page_title="서라벌여중 시간표·결보강 관리 시스템",
    page_icon="📘",
    layout="wide",
...
def get_worksheet(spreadsheet_id: str, sheet_name: str):
    try:
        ...
        st.error(f"Google API 할당량 초과 또는 네트워크 오류 ({sheet_name}): {e}")
        return None
    except Exception as e:
...
def df_to_worksheet(ws, df: pd.DataFrame):
    ...
    try:
        ...
        st.error(f"Google Sheets API 쓰기 에러: {e}")
    except Exception as e:
        st.error(f"시트 저장 처리 실패: {e}")

# ==========================================================================================
# 1. 히스토리 (Undo / Redo) 및 메모리 최적화
# ==========================================================================================
def push_history(action_name: str = "작업"):
    ...
    snapshot = {
        ...
        "absences": st.session_state.absences.copy() if hasattr(st.session_state, "absences") else pd.DataFrame(),
        "subs": st.session_state.subs.copy() if hasattr(st.session_state, "subs") else pd.DataFrame(),
        "swaps": st.session_state.swaps.copy() if hasattr(st.session_state, "swaps") else pd.DataFrame(),
        "part_time": st.session_state.part_time.copy() if hasattr(st.session_state, "part_time") else pd.DataFrame(),
    }
...
def undo():
    ...
    st.session_state.absences = snap["absences"].copy()
    st.session_state.subs = snap["subs"].copy()
    st.session_state.swaps = snap["swaps"].copy()
    st.session_state.part_time = snap.get("part_time", st.session_state.part_time).copy()
    return True
...
# ==========================================================================================
# 3. 핵심 로직 (맞교환 동적 반영, 결강·보강 실시간 동기화)
# ==========================================================================================
def get_effective_timetable_for_date(on_date: str) -> pd.DataFrame:
    """맞교환(swaps) 내역을 완벽히 반영하여 특정 날짜의 변동된 시간표 반환"""
    ...
    if not swaps.empty:
        for _, sw in swaps.iterrows():
            orig_date = normalize_date_str(sw.get("원본일자", ""))
            target_date = normalize_date_str(sw.get("목표일자", ""))
            swap_type = str(sw.get("유형", "")).strip()

            t_a = str(sw.get("교사A", "")).strip()
            p_a = safe_int(sw.get("교시A"))
            c_a = str(sw.get("학급A", "")).strip()
            s_a = str(sw.get("과목A", "")).strip()

            t_b = str(sw.get("교사B", "")).strip()
            p_b = safe_int(sw.get("교시B"))
            c_b = str(sw.get("학급B", "")).strip()
            s_b = str(sw.get("과목B", "")).strip()
...
def is_free(teacher: str, day: str, period: int, on_date: str = None, is_part_time: bool = False, eff_tt: pd.DataFrame = None) -> bool:
    ...
    if not is_part_time:
        if norm_date:
            ...
            tt = st.session_state.timetable
            if not tt.empty and not tt[(tt["교사명"] == teacher) & (tt["요일"] == day) & (tt["교시"] == p_int)].empty:
                return False
    if norm_date and not is_part_time:
        s = st.session_state.subs
        if not s.empty and "일자" in s.columns and "교시" in s.columns:
            s_date = s["일자"].apply(normalize_date_str)
            if ((s_date == norm_date) & (s["교시"].apply(safe_int) == p_int) & (s["보강교사"] == teacher)).any():
                return False
        a = st.session_state.absences
        if not a.empty and "일자" in a.columns and "교시" in a.columns:
            a_date = a["일자"].apply(normalize_date_str)
            if ((a_date == norm_date) & (a["교사명"] == teacher) & (a["교시"].apply(safe_int) == p_int)).any():
                return False
    return True
...
def absent_all_day(teacher: str, on_date: str) -> bool:
    ...
    if a.empty or "일자" not in a.columns:
        return False
    norm_date = normalize_date_str(on_date)
    return ((a["일자"].apply(normalize_date_str) == norm_date) & (a["교사명"] == teacher)).any()
...
def recommend_substitutes(day: str, period: int, subject: str, class_name: str,
    ...
    for t in teachers["교사명"].tolist():
        if t == absent_teacher or absent_all_day(t, norm_date):
            continue
        if not is_free(t, day, period, norm_date, eff_tt=eff_tt):
            continue
...
def get_teacher_week_view(teacher: str, ref_date: date):
    """특정 주의 변동 반영 주간 시간표 생성 (get_effective_timetable_for_date 통합)"""
    ...
    for d_idx, cur_date in enumerate(week_dates):
        ...
        if not abs_m.empty and "일자" in abs_m.columns:
            m = abs_m[(abs_m["일자"].apply(normalize_date_str) == cur_date_str) & (abs_m["교사명"] == teacher)]
        ...
        if not sub_m.empty and "일자" in sub_m.columns:
            m = sub_m[(sub_m["일자"].apply(normalize_date_str) == cur_date_str) & (sub_m["보강교사"] == teacher)]
            for _, r in m.iterrows():
                p = safe_int(r.get("교시"))
                if 1 <= p <= MAX_PERIOD:
...
def build_report_html(on_date: str) -> str:
    ...
    a = a[a["일자"].apply(normalize_date_str) == norm_date] if not a.empty and "일자" in a.columns else pd.DataFrame()
    s = s[s["일자"].apply(normalize_date_str) == norm_date] if not s.empty and "일자" in s.columns else pd.DataFrame()
    w = w[(w["원본일자"].apply(normalize_date_str) == norm_date) | (w["목표일자"].apply(normalize_date_str) == norm_date)] if not w.empty and "원본일자" in w.columns else pd.DataFrame()
...
with st.sidebar:
    ...
    theme_choice = st.selectbox(
        "테마 선택 (색맹/시력 보호 고려)",
        ["기본 라이트 (Light)", "고대비/색맹 포용 (High Contrast)", "모던 다크 (Modern Dark)", "소프트 오션 (Ocean Blue)"],
        index=0
    )
...
with tabs[0]:
    ...
    if view == "교사별 전체 매트릭스 (기본 주간)":
        st.dataframe(teacher_matrix(), use_container_width=True, height=720)
...
with tabs[3]:
    ...
    if day not in DAYS:
        ...
        if todays.empty:
            st.info(f"{who} 교사는 {on_date} ({day}요일)에 수업이 없습니다 (맞교환 변경 반영됨).")
        else:
            opts = [f"{safe_int(r['교시'])}교시 · {r['학급']} · {r['과목']}" for _, r in todays.iterrows()]
            picked = st.multiselect("결강 교시 선택 (맞교환된 수업 포함)", opts, default=opts)

            if st.button("결강 등록 실행", type="primary"):
                if not picked:
                    ...
                    try:
                        ...
                        if not a.empty and "일자" in a.columns:
                            a = a[~((a["일자"].apply(normalize_date_str) == on_date) & (a["교사명"] == who))]
                        new_rows = []
                        for _, r in rows.iterrows():
                            new_rows.append({
    ...
    if not ab.empty:
        show = ab[ab["일자"].apply(normalize_date_str) == on_date] if (st.checkbox("선택한 일자만 보기", value=True) and "일자" in ab.columns) else ab
    ...
    if ab.empty or "결강ID" not in ab.columns:
        ...
        if not unique_cids:
            ...
            if rows.empty:
                ...
                st.markdown(f"### 📌 {norm_head_date} ({head.get('요일','')}) · **{head.get('교사명','')}** · 사유: {head.get('사유','')} · {len(rows)}시간")
                include_pt = st.checkbox("시간강사도 추천 대상에 포함", value=False)

                if st.button("전 교시 자동 추천 및 일괄 배정", type="primary"):
                    try:
                ...
                for _, r in rows.iterrows():
                    ...
                    with st.expander(f"📘 {p}교시 · {r.get('학급','')} · {r.get('과목','')}", expanded=True):
                        ...
                        if assigned:
                            st.success(f"현재 배정 교사: **{assigned}**")
                            if st.button("❌ 배정 취소 (미배정 상태로 되돌리기)", key=f"cancel_{cid}_{p}", type="secondary"):
                            ...
                            if cand.empty:
                                ...
                                cc = st.columns([2, 2, 1])
                                pick = cc[0].selectbox("보강 교사 선택", cand["보강교사"].tolist(), key=f"pick_{cid}_{p}")
                                memo = cc[1].text_input("비고", key=f"memo_{cid}_{p}")
                                if cc[2].button("배정 저장", key=f"btn_{cid}_{p}", use_container_width=True):
                                    try:
...
with tabs[5]:
    ...
    st.caption("맞교환 변경은 지정된 날짜의 시간표에만 적용됩니다.")
    ...
    with col_a:
        ...
        if not teacher_list:
            ...
            if sub_a.empty:
                ...
                sel_a = st.selectbox("변경할 수업 선택", opts_a, key="lesson_a_select")
...
with tabs[6]:
    ...
    s = st.session_state.subs
    xls = to_excel_bytes({
        "결강": a[a["일자"].apply(normalize_date_str) == rd] if not a.empty and "일자" in a.columns else pd.DataFrame(),
        "보강": s[s["일자"].apply(normalize_date_str) == rd] if not s.empty and "일자" in s.columns else pd.DataFrame(),
...
with tabs[7]:
    ...
    c = st.columns(4)
    c[0].metric("총 보강 건수", int(df["누적보강"].sum()) if not df.empty else 0)
    c[1].metric("보강 참여 교사", int((df["누적보강"] > 0).sum()) if not df.empty else 0)
    c[2].metric("최다 보강 시수", int(df["누적보강"].max()) if not df.empty else 0)
    c[3].metric("평균 보강 시수", round(float(df["누적보강"].mean()), 2) if not df.empty else 0)
    ...
    st.divider()
    if st.button("📊 현재 통계를 Google Sheets 누적보강 시트에 저장", type="primary"):
        save_df = df.copy()
...
# ------------------------------------------------------------------ 8. 실시간 시뮬레이션 테스트용 (시트 미반영)
with tabs[8]:
    st.subheader("🧪 시간표 변경 실시간 테스트 (Google Sheets 저장 안됨)")
    st.info("이 탭은 변경 사항을 실제 저장하지 않고 안전하게 가상 테스트할 수 있는 공간입니다. 실시간 상태를 동적으로 반영합니다.")
    ...
    if not teacher_list:
        ...
        st.caption(f"주간: {week_dates[0].strftime('%Y-%m-%d')} ~ {week_dates[4].strftime('%Y-%m-%d')} (결강·보강·맞교환 실시간 반영됨)")

        st.dataframe(grid, use_container_width=True, height=400)

        st.divider()
        st.markdown("#### 🔄 가상 맞교환 시뮬레이션")
        ...
        if pick_test_a:
            st.markdown(f"##### 💡 가상 시뮬레이션 결과 ({pick_test_a['일자']} {pick_test_a['교시']}교시 → {test_date_b_str} {test_p_b}교시)")
...
# ------------------------------------------------------------------ 9. 변경된 교사 주간표
with tabs[9]:
    st.subheader("📅 변경된 교사 주간 시간표 (결강·보강·맞교환 즉시 반영)")
    teacher_list = st.session_state.teachers["교사명"].tolist() if not st.session_state.teachers.empty else []
    if not teacher_list:
        st.warning("등록된 교사가 없습니다.")
    else:
...
with tabs[10]:
    st.subheader("🤖 교사 복무 기반 수업 처리(교체 vs 보강) 자동 판단 및 즉시 맞교환")
    ...
    if abs_day not in DAYS:
        ...
        if target_lessons.empty:
            ...
            st.markdown(f"#### 📌 {target_teacher} 교사의 {abs_day}요일 수업 목록 ({len(target_lessons)}시간 - 맞교환 반영됨)")
            ...
            for _, les in target_lessons.iterrows():
                ...
                if not swaps_df.empty and "원본일자" in swaps_df.columns:
                    s_orig = swaps_df["원본일자"].apply(normalize_date_str)
                    s_target = swaps_df["목표일자"].apply(normalize_date_str)
                    cond_orig = (s_orig == date_str) & (swaps_df["교사A"] == target_teacher) & (swaps_df["교시A"].apply(safe_int) == p_val)
                    cond_target_b = (s_target == date_str) & (swaps_df["교사B"] == target_teacher) & (swaps_df["교시B"].apply(safe_int) == p_val)
                    cond_target_a = (s_target == date_str) & (swaps_df["교사A"] == target_teacher) & (swaps_df["교시B"].apply(safe_int) == p_val)
                    if (cond_orig | cond_target_b | cond_target_a).any():
                ...
                if not subs_df.empty and "일자" in subs_df.columns and "교시" in subs_df.columns and "결강교사" in subs_df.columns:
                    s_date = subs_df["일자"].apply(normalize_date_str)
                    m_sub = subs_df[(s_date == date_str) & (subs_df["교시"].apply(safe_int) == p_val) & (subs_df["결강교사"] == target_teacher)]
                ...
                with st.expander(f"📘 {p_val}교시 | 학급: {cls_val} | 과목: {sub_val}", expanded=True):
                    ...
                    cands = recommend_substitutes(abs_day, p_val, sub_val, cls_val, target_teacher, date_str, top_n=10, eff_tt=eff_tt)
                    available_free_teachers = len(cands)
                    
                    possible_swaps_all = []
                    max_p = PERIODS_PER_DAY.get(abs_day, 7)
                    for other_p in range(1, max_p + 1):
                        if other_p != p_val and is_free(target_teacher, abs_day, other_p, date_str, eff_tt=eff_tt):
                    ...
                    if swap_prob >= 60:
                        st.success(f"💡 **[수업 교체 권장]** 사전 신청 복무이며, 맞교환 가능한 타 교사 수업이 {swap_possible_count}건 존재합니다.")
                    elif sub_prob >= 60:
                        st.warning(f"💡 **[보강 처리 권장]** 긴급도 및 여건상 공강 교사 보강 배정이 적합합니다.")
                    else:
                        st.info("💡 **[중립/시간강사 권장]** 교체와 보강 처리 가능성이 유사합니다.")

                    st.caption(f"· 동시간대 공강 교사: {available_free_teachers}명 | · 해당 요일 직접 교체 가능 수업: {swap_possible_count}개")

                    st.markdown("##### 🔄 교체 가능한 수업 목록 (즉시 맞교환)")
                    ...
                    if possible_swaps_all:
                        ...
                        if st.button("⚡ 선택한 수업과 즉시 맞교환 실행", key=f"btn_exec_sw_{p_val}", type="primary"):
                            ...
                            if target_sw.get("유형") in ["1:1맞교환", "직접1:1"]:
                                ...
                                st.success(f"✅ {target_teacher} ↔ {b_info.get('교사명', target_sw.get('교사B', ''))} 수업 맞교환이 완료되었습니다!")
                            else:
                        ...
                        st.info("ℹ️ 해당 요일에 충돌 없이 맞교환할 대상 수업이 없습니다. '보강 배정' 탭을 활용하세요.")
