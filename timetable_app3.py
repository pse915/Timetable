...
with tabs[10]:
    st.subheader("🤖 교사 복무 기반 수업 처리(교체 vs 보강) 자동 판단 및 확률 추천")
    st.caption("갑작스러운 복무 발생 시, 사유 및 조건에 따른 수업 맞교환(교체) 적합성과 보강 처리 적합성을 확률로 분석합니다.")

    col1, col2 = st.columns(2)
    with col1:
        target_teacher = st.selectbox("대상 교사 선택", st.session_state.teachers["교사명"].tolist() if not st.session_state.teachers.empty else ["없음"], key="rec_teacher")
    ...
    if abs_day not in DAYS:
        st.warning("주말은 시간표가 없습니다.")
    elif target_teacher == "없음":
        st.warning("선택된 교사가 없습니다.")
    else:
        tt = st.session_state.timetable
        target_lessons = tt[(tt["교사명"] == target_teacher) & (tt["요일"] == abs_day)].sort_values("교시")
        ...
        if target_lessons.empty:
            ...
            for _, les in target_lessons.iterrows():
                ...
                with st.expander(f"📘 {p_val}교시 | 학급: {cls_val} | 과목: {sub_val}", expanded=True):
                    ...
                    # 1:1 맞교환/연계 가능 가능성 분석 (단순 추정)
                    # 타 교시 공강 여부 체크
                    swap_possible_count = 0
                    for other_p in range(1, PERIODS_PER_DAY.get(abs_day, 7) + 1):
                        if other_p != p_val and is_free(target_teacher, abs_day, other_p, date_str):
                            # target_teacher가 이동 가능한 공강 시간이 있는 경우
                            df_sw, df_lk, _ = get_target_time_recommendations(target_teacher, date_str, p_val, cls_val, sub_val, date_str, other_p)
                            if not df_sw.empty or not df_lk.empty:
                                swap_possible_count += 1
                    ...
                    if swap_prob >= 60:
                        st.success(f"💡 **[수업 교체 권장]** {duty_reason} 사유로 인한 사전 변경이며, 교체 가능한 타 교시/교사가 존재합니다. '시간표 맞교환' 탭을 사용하여 수업을 교체하세요.")
                    elif sub_prob >= 60:
                        st.warning(f"💡 **[보강 처리 권장]** {duty_reason} 복무의 긴급도나 맞교환 여건을 고려할 때, 당일 공강 교사 배정(보강) 처리하는 것이 적합합니다.")
                    else:
                        st.info("💡 **[중립/시간강사 권장]** 교체와 보강 처리 가능성이 비슷합니다. 학교 여건이나 시간강사 채용 여부를 함께 고려하세요.")

                    # 세부 근거
                    st.caption(f"· 동시간대 공강 교사 수: {available_free_teachers}명 | · 해당 주간 교체 가능 타 시간대: {swap_possible_count}개")

                    # 원클릭 바로가기 버튼
                    act_c1, act_c2 = st.columns(2)
                    if act_c1.button("🔄 맞교환 메뉴로 바로가기", key=f"btn_go_swap_{p_val}"):
                        st.info("상단 메뉴의 **'시간표 맞교환 & 변경 추천'** 탭을 선택해 진행하세요.")
                    if act_c2.button("➕ 결강 등록 및 보강 배정", key=f"btn_go_abs_{p_val}"):
                        cid = f"{date_str}-{target_teacher}"
                        push_history(f"자동추천 결강등록 ({target_teacher})")
                        a = st.session_state.absences
                        new_r = {
                            "결강ID": cid, "일자": date_str, "요일": abs_day, "교사명": target_teacher,
                            "사유": duty_reason, "상세사유": f"자동추천기능등록({urgency_type})", "교시": p_val,
                            "학급": cls_val, "과목": sub_val, "등록시각": datetime.now().strftime("%Y-%m-%d %H:%M")
                        }
                        st.session_state.absences = pd.concat([a, pd.DataFrame([new_r])], ignore_index=True)
                        save_work_data_to_gsheet()
