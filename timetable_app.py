...
==========================================================================================
 ...
 2026 서라벌여자중학교용 – 연산 성능 최적화 & 대용량/고속 처리 강화 버전
==========================================================================================
"""
...
def save_history_log_to_gsheet():
    try:
        ...
        if ws:
            df_to_worksheet(ws, pd.DataFrame(logs))
    except Exception as e:
...
def load_timetable_from_gsheet():
    try:
        sh = get_spreadsheet(TIMETABLE_SHEET_ID)
        ti = df_from_worksheet(sh.worksheet("교사정보"))
        tt = df_from_worksheet(sh.worksheet("시간표"))
...
def load_work_data_from_gsheet():
    try:
        sh = get_spreadsheet(WORK_SHEET_ID)
        absences = df_from_worksheet(sh.worksheet("결강"))
        subs = df_from_worksheet(sh.worksheet("보강"))
        swaps = df_from_worksheet(sh.worksheet("맞교환"))
        part_time = df_from_worksheet(sh.worksheet("시간강사"))
        cumulative = df_from_worksheet(sh.worksheet("누적보강"))

        if not absences.empty:
            if "교시" in absences.columns: absences["교시"] = absences["교시"].apply(lambda x: safe_int(x, 0))
            if "일자" in absences.columns: absences["일자"] = absences["일자"].apply(normalize_date_str)
        if not subs.empty:
            if "교시" in subs.columns: subs["교시"] = subs["교시"].apply(lambda x: safe_int(x, 0))
            if "일자" in subs.columns: subs["일자"] = subs["일자"].apply(normalize_date_str)
        if not swaps.empty:
            if "원본일자" in swaps.columns: swaps["원본일자"] = swaps["원본일자"].apply(normalize_date_str)
            if "목표일자" in swaps.columns: swaps["목표일자"] = swaps["목표일자"].apply(normalize_date_str)
            if "교시A" in swaps.columns: swaps["교시A"] = swaps["교시A"].apply(lambda x: safe_int(x, 0))
            if "교시B" in swaps.columns: swaps["교시B"] = swaps["교시B"].apply(lambda x: safe_int(x, 0))
...
def save_work_data_to_gsheet():
    try:
        sh = get_spreadsheet(WORK_SHEET_ID)
        df_to_worksheet(sh.worksheet("결강"), st.session_state.absences)
        df_to_worksheet(sh.worksheet("보강"), st.session_state.subs)
        df_to_worksheet(sh.worksheet("맞교환"), st.session_state.swaps)
        df_to_worksheet(sh.worksheet("시간강사"), st.session_state.part_time)
...
def save_cumulative_to_gsheet(df: pd.DataFrame):
    try:
        ws = get_worksheet(WORK_SHEET_ID, "누적보강")
...
def get_effective_timetable_for_date(on_date: str) -> pd.DataFrame:
    ...
    if tt.empty:
        return pd.DataFrame(columns=REQUIRED_TT_COLS + ["과목군"])

    base = tt[tt["요일"] == day].copy()
    current_map = {}
    for _, r in base.iterrows():
        ...
        current_map[(t, p)] = {
            ...
            "과목": str(r["과목"]).strip(),
            "학급": str(r["학급"]).strip(),
            "과목군": str(r.get("과목군", subject_group(r["과목"]))).strip()
        }

    swaps = st.session_state.get("swaps", pd.DataFrame())
    if not swaps.empty and "원본일자" in swaps.columns:
        relevant_swaps = swaps[(swaps["원본일자"] == norm_on_date) | (swaps["목표일자"] == norm_on_date)]
        for _, sw in relevant_swaps.iterrows():
            orig_date = sw.get("원본일자", "")
            target_date = sw.get("목표일자", "")
            swap_type = str(sw.get("유형", "")).strip()

            t_a, p_a = str(sw.get("교사A", "")).strip(), safe_int(sw.get("교시A"))
            c_a, s_a = str(sw.get("학급A", "")).strip(), str(sw.get("과목A", "")).strip()
            t_b, p_b = str(sw.get("교사B", "")).strip(), safe_int(sw.get("교시B"))
            c_b, s_b = str(sw.get("학급B", "")).strip(), str(sw.get("과목B", "")).strip()
    ...
    return df_eff if not df_eff.empty else pd.DataFrame(columns=REQUIRED_TT_COLS + ["과목군"])

def build_availability_map(source_df: pd.DataFrame, name_col: str) -> dict:
    """교사별 가능/불가능 시간 고속 조회 딕셔너리 생성"""
    avail_map = {}
    if source_df.empty or name_col not in source_df.columns:
        return avail_map
    for _, row in source_df.iterrows():
        t_name = str(row[name_col]).strip()
        avail_map[t_name] = {}
        for col in source_df.columns:
            ...
            is_unavail = str_val in ['0', '0.0', 'False', 'false', '불가', '불가능', 'N', 'n', 'X', 'x', '없음']
            try:
                num_val = pd.to_numeric(val, errors='coerce')
                if pd.notna(num_val) and num_val <= 0: is_unavail = True
            except Exception:
                pass
            avail_map[t_name][col] = not is_unavail
    return avail_map

def is_teacher_available(teacher: str, day: str, period: int, is_part_time: bool = False, avail_map: dict = None) -> bool:
    if avail_map is None:
        ...
        avail_map = build_availability_map(source, name_col)

    t_map = avail_map.get(teacher, {})
...
def is_free(teacher: str, day: str, period: int, on_date: str = None, is_part_time: bool = False, eff_tt: pd.DataFrame = None) -> bool:
...
def can_teacher_take_slot(teacher: str, day: str, period: int, on_date: str = None, is_part_time: bool = False, eff_tt: pd.DataFrame = None) -> bool:
    return is_teacher_available(teacher, day, period, is_part_time) and is_free(teacher, day, period, on_date, is_part_time, eff_tt=eff_tt)
...
def absent_all_day(teacher: str, on_date: str) -> bool:
    ...
    if a.empty or "일자" not in a.columns:
        return False
    norm_date = normalize_date_str(on_date)
    return ((a["일자"] == norm_date) & (a["교사명"] == teacher)).any()
...
def cumulative_sub_count(start_date: str = None, end_date: str = None) -> dict:
    ...
    if not s.empty and "보강교사" in s.columns:
        if start_date and end_date and "일자" in s.columns:
            s_date = s["일자"]
            s = s[(s_date >= normalize_date_str(start_date)) & (s_date <= normalize_date_str(end_date))]
...
def recommend_substitutes(day: str, period: int, subject: str, class_name: str,
    ...
    # 미리 조회 맵 구성 (반복 성능 극대화)
    teacher_avail_map = build_availability_map(teachers, "교사명")
    part_avail_map = build_availability_map(part, "시간강사명") if include_part_time else {}

    # 공강 교사 필터링
    eff_busy_teachers = set(eff_tt[eff_tt["교시"] == safe_int(period)]["교사명"]) if not eff_tt.empty else set()
    
    s = st.session_state.subs
    sub_busy_teachers = set(s[(s["일자"] == norm_date) & (s["교시"] == safe_int(period))]["보강교사"]) if not s.empty and "일자" in s.columns and "교시" in s.columns else set()

    a = st.session_state.absences
    absent_day_teachers = set(a[a["일자"] == norm_date]["교사명"]) if not a.empty and "일자" in a.columns else set()

    rows = []

    for t in teachers["교사명"].tolist():
        if t == absent_teacher or t in absent_day_teachers:
            continue
        if t in eff_busy_teachers or t in sub_busy_teachers:
            continue
        if not is_teacher_available(t, day, period, is_part_time=False, avail_map=teacher_avail_map):
            continue
    ...
    if include_part_time and not part.empty and "시간강사명" in part.columns:
        for t in part["시간강사명"].dropna().tolist():
            if not is_teacher_available(t, day, period, is_part_time=True, avail_map=part_avail_map):
                continue
...
def check_conflicts(tt: pd.DataFrame) -> pd.DataFrame:
    ...
    avail_map = build_availability_map(st.session_state.teachers, "교사명")
    for _, r in tt.iterrows():
        p_int = safe_int(r["교시"])
        if not is_teacher_available(r["교사명"], r["요일"], p_int, avail_map=avail_map):
...
def get_target_time_recommendations(teacher_a, date_a_str, period_a, class_a, subject_a, date_b_str, period_b, eff_tt_a=None, eff_tt_b=None):
    ...
    if not df_swap.empty: df_swap = df_swap.sort_values("_score", ascending=False).reset_index(drop=True)
    df_linked = pd.DataFrame(linked_recs)
    if not df_linked.empty: df_linked = df_linked.sort_values("_score", ascending=False).reset_index(drop=True)
    df_sub = pd.DataFrame(sub_recs)
    if not df_sub.empty: df_sub = df_sub.sort_values(["_prio", "_score"], ascending=[True, False]).reset_index(drop=True)
...
def get_teacher_week_view(teacher: str, ref_date: date):
    ...
    for d_idx, cur_date in enumerate(week_dates):
        ...
        d = DAYS[d_idx]
        eff_tt = get_effective_timetable_for_date(cur_date_str)
        
        if not eff_tt.empty:
            t_lessons = eff_tt[eff_tt["교사명"] == teacher]
            for _, r in t_lessons.iterrows():
                ...
                if 1 <= p <= PERIODS_PER_DAY.get(d, 7):
                    ...
                    if not swaps_df.empty:
                        ...
                        if (c1 | c2).any(): is_swapped_slot = True
...
def get_changed_teachers_for_week(ref_date: date) -> list:
    ...
    if not ab.empty and "일자" in ab.columns and "교사명" in ab.columns:
        changed.update(ab[ab["일자"].isin(week_date_strs)]["교사명"].dropna().unique().tolist())
        
    sb = st.session_state.get("subs", pd.DataFrame())
    if not sb.empty and "일자" in sb.columns:
        m_sb = sb[sb["일자"].isin(week_date_strs)]
        if "보강교사" in m_sb.columns: changed.update(m_sb["보강교사"].dropna().unique().tolist())
        if "결강교사" in m_sb.columns: changed.update(m_sb["결강교사"].dropna().unique().tolist())
            
    sw = st.session_state.get("swaps", pd.DataFrame())
    if not sw.empty:
        if "원본일자" in sw.columns:
            m_sw1 = sw[sw["원본일자"].isin(week_date_strs)]
            if "교사A" in m_sw1.columns: changed.update(m_sw1["교사A"].dropna().unique().tolist())
            if "교사B" in m_sw1.columns: changed.update(m_sw1["교사B"].dropna().unique().tolist())
        if "목표일자" in sw.columns:
            m_sw2 = sw[sw["목표일자"].isin(week_date_strs)]
            if "교사A" in m_sw2.columns: changed.update(m_sw2["교사A"].dropna().unique().tolist())
            if "교사B" in m_sw2.columns: changed.update(m_sw2["교사B"].dropna().unique().tolist())
    ...
    return sorted(list(set([str(t).strip() for t in changed if str(t).strip() in all_teachers])))
...
def build_report_html(on_date: str) -> str:
    norm_date = normalize_date_str(on_date)
    day = WEEKDAY_KR[datetime.strptime(norm_date, "%Y-%m-%d").weekday()]
    a = st.session_state.absences
    s = st.session_state.subs
    w = st.session_state.swaps
    a = a[a["일자"] == norm_date] if not a.empty and "일자" in a.columns else pd.DataFrame()
    s = s[s["일자"] == norm_date] if not s.empty and "일자" in s.columns else pd.DataFrame()
    w = w[(w["원본일자"] == norm_date) | (w["목표일자"] == norm_date)] if not w.empty and "원본일자" in w.columns else pd.DataFrame()

    def rows_abs():
        if a.empty: return "<tr><td colspan='6' class='empty'>해당 일자의 결강 등록 내역이 없습니다.</td></tr>"
        out = []
    ...
    def rows_sub():
        if s.empty: return "<tr><td colspan='7' class='empty'>보강 배정 내역이 없습니다.</td></tr>"
        out = []
    ...
    def rows_swap():
        if w.empty: return "<tr><td colspan='4' class='empty'>시간표 맞교환 내역이 없습니다.</td></tr>"
        out = []
...
with st.sidebar:
    ...
    if c_undo.button("↩ 되돌리기", use_container_width=True):
        if undo():
            save_work_data_to_gsheet()
    ...
    if c_redo.button("↪ 다시실행", use_container_width=True):
        if redo():
            save_work_data_to_gsheet()
...
with tabs[8]:
    ...
    st.info("시트에 영향 없이 자유롭게 변경 시나리오를 테스트하는 공간입니다.")
    ...
    if not teacher_list:
        ...
        if pick_test_a:
            ...
            with t_swap_test:
                if df_swap_t.empty: st.info("추천할 1:1 맞교환 대상이 없습니다.")
                else: st.dataframe(df_swap_t.drop(columns=["b_info", "errs", "guides", "is_valid", "_score"], errors='ignore'), use_container_width=True, hide_index=True)
                    
            with t_linked_test:
                if df_linked_t.empty: st.info("추천할 연계 교환 대상이 없습니다.")
                else: st.dataframe(df_linked_t.drop(columns=["b_info", "errs", "guides", "is_valid", "_score"], errors='ignore'), use_container_width=True, hide_index=True)
                    
            with t_sub_test:
                if df_sub_t.empty: st.info("추천할 대리/보강 교사가 없습니다.")
                else: st.dataframe(df_sub_t.drop(columns=["_prio", "_score"], errors='ignore'), use_container_width=True, hide_index=True)
...
with tabs[9]:
    ...
    if not changed_teachers:
        st.success("🎉 선택하신 주차에는 시간표 변경 내역이 있는 교사가 없습니다.")
        teacher_list = st.session_state.teachers["교사명"].tolist() if not st.session_state.teachers.empty else []
        if teacher_list:
        ...
        st.info(f"💡 이번 주차 변경 교사: 총 **{len(changed_teachers)}명** ({', '.join(changed_teachers)})")
        ...
        if view_mode == "전체 변경 교사 한눈에 모아보기 (권장)":
            for t in changed_teachers:
                with st.expander(f"👤 **{t} 선생님** 주간 변경 시간표", expanded=True):
...
# ------------------------------------------------------------------ 10. 복무 기반 자동 판단 및 맞교환 (복원 & 완전 완성)
with tabs[10]:
    st.subheader("🤖 교사 복무 기반 수업 처리(교체 vs 보강) 자동 판단 및 바로 맞교환")
    st.caption("사유 및 긴급도에 따라 수업 맞교환/보강 방안을 자동 판단합니다.")
    ...
    if abs_day not in DAYS:
        ...
        eff_tt = get_effective_timetable_for_date(date_str)
        target_lessons = eff_tt[eff_tt["교사명"] == target_teacher].sort_values("교시") if not eff_tt.empty else pd.DataFrame()
        
        if target_lessons.empty:
            st.info(f"{target_teacher} 교사는 {date_str} ({abs_day})에 예정된 수업이 없습니다.")
        else:
            st.markdown(f"#### 📌 {target_teacher} 교사의 {abs_day}요일 수업 목록 ({len(target_lessons)}시간)")
            
            subs_df = st.session_state.subs
            swaps_df = st.session_state.swaps
            ...
            for _, les in target_lessons.iterrows():
                ...
                is_already_swapped = False
                if not swaps_df.empty and "원본일자" in swaps_df.columns:
                    s_orig, s_target = swaps_df["원본일자"], swaps_df["목표일자"]
                    cond_orig = (s_orig == date_str) & (swaps_df["교사A"] == target_teacher) & (swaps_df["교시A"] == p_val)
                    cond_target = (s_target == date_str) & ((swaps_df["교사B"] == target_teacher) | (swaps_df["교사A"] == target_teacher)) & (swaps_df["교시B"] == p_val)
                    if (cond_orig | cond_target).any():
                        is_already_swapped = True

                is_assigned, assigned_teacher = False, ""
                if not subs_df.empty and "일자" in subs_df.columns and "교시" in subs_df.columns:
                    m_sub = subs_df[(subs_df["일자"] == date_str) & (subs_df["교시"] == p_val) & (subs_df["결강교사"] == target_teacher)]
                    if not m_sub.empty and str(m_sub.iloc[0].get("보강교사", "")).strip():
                        is_assigned = True
                        assigned_teacher = str(m_sub.iloc[0].get("보강교사", "")).strip()

                label = f"📘 {p_val}교시 | {cls_val} | {sub_val}"
                if is_assigned: label += f" (✅ 보강배정: {assigned_teacher})"
                elif is_already_swapped: label += " (🔄 맞교환 완료)"

                with st.expander(label, expanded=not (is_assigned or is_already_swapped)):
                    if is_assigned:
                        st.success(f"현재 **{assigned_teacher}** 선생님이 보강 배정되어 있습니다.")
                        if st.button("❌ 보강 취소", key=f"duty_cancel_{p_val}"):
                            cancel_substitute(f"{date_str}-{target_teacher}", p_val)
                            st.rerun()
                    elif is_already_swapped:
                        st.info("이 수업은 이미 시간표 변경/맞교환이 처리되었습니다.")
                    else:
                        st.write(f"**권장 처리 방식**: {'대리/보강 배정 권장 (긴급)' if urgency_type == '당일 급구/사후 신청 (긴급)' else '사전 맞교환 권장'}")
                        c_d1, c_d2 = st.columns(2)
                        
                        target_day_b = st.selectbox("희망 요일", DAYS, index=DAYS.index(abs_day), key="duty_day_b_" + str(p_val))
                        target_p_b = st.selectbox("희망 교시", list(range(1, PERIODS_PER_DAY[target_day_b] + 1)), key="duty_pb_" + str(p_val))
                        
                        df_s, df_l, df_sub = get_target_time_recommendations(
                            target_teacher, date_str, p_val, cls_val, sub_val, date_str, target_p_b
                        )
                        if not df_s.empty and df_s.iloc[0]["is_valid"]:
                            best = df_s.iloc[0]
                            st.success(f"추천 맞교환: {best['교사B']} ({best['현재 수업']})")
                            if st.button(f"'{best['교사B']}' 교사 및 맞교환 확정", key=f"btn_duty_swap_{p_val}", type="primary"):
                                do_swap({"교사명": target_teacher, "일자": date_str, "요일": abs_day, "교시": p_val, "학급": cls_val, "과목": sub_val},
                                        best["b_info"], date_str, date_str, False)
                                st.success("맞교환 처리 완료")
                                st.rerun()
                        elif not df_sub.empty:
                            best_sub = df_sub.iloc[0]
                            st.info(f"추천 보강 교사: {best_sub['보강/대리 교사']} ({best_sub['우선순위']})")
                            if st.button(f"'{best_sub['보강/대리 교사']}' 보강 배정", key=f"btn_duty_sub_{p_val}", type="primary"):
                                add_substitute(f"{date_str}-{target_teacher}", date_str, abs_day, p_val, cls_val, sub_val,
                                               target_teacher, best_sub["보강/대리 교사"], "복무자동보강", best_sub["우선순위"], duty_reason)
                                st.success("보강 배정 완료")
                                st.rerun()
                        else:
                            st.warning("적절한 맞교환/보강 교사를 찾지 못했습니다.")
