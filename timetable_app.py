...
==========================================================================================
 ...
 2026 서라벌여자중학교용 – 백업 / 달력 주간표 / 시간강사 / 학기별 통계 통합 버전
------------------------------------------------------------------------------------------
 실행 방법
...
def restore_from_backup_xlsx(file_bytes: bytes):
    """앱이 내보낸 결보강내역 xlsx를 복원"""
    try:
        xls = pd.ExcelFile(io.BytesIO(file_bytes))
        restored = []
        if "결강" in xls.sheet_names:
            df = pd.read_excel(xls, "결강")
            if not df.empty:
                st.session_state.absences = df
                restored.append("결강")
        if "보강" in xls.sheet_names:
            df = pd.read_excel(xls, "보강")
            if not df.empty:
                st.session_state.subs = df
                restored.append("보강")
        if "시간표맞교환" in xls.sheet_names:
            df = pd.read_excel(xls, "시간표맞교환")
            if not df.empty:
                st.session_state.swaps = df
                restored.append("맞교환")
        if restored:
            return True, f"복원 완료: {', '.join(restored)}"
        return False, "복원할 데이터가 없습니다."
...
def do_swap(a: dict, b: dict, date_a: str, date_b: str, is_part_time_purpose: bool = False):
    tt = st.session_state.timetable.copy()
    ia = tt[(tt["교사명"] == a["교사명"]) & (tt["요일"] == a["요일"]) & (tt["교시"] == int(a["교시"]))].index
    ib = tt[(tt["교사명"] == b["교사명"]) & (tt["요일"] == b["요일"]) & (tt["교시"] == int(b["교시"]))].index
    if len(ia) == 0 or len(ib) == 0:
        return False
    ia, ib = ia[0], ib[0]
    tt.loc[ia, ["요일", "교시"]] = [b["요일"], int(b["교시"])]
    tt.loc[ib, ["요일", "교시"]] = [a["요일"], int(a["교시"])]
    st.session_state.timetable = tt.reset_index(drop=True)
    rec = {
...
def do_linked_swap(a: dict, teacher_b: str, date_a: str, date_b: str, day_b: str, period_b: int,
    ...
    if len(ia) == 0:
        return False
    ia = ia[0]
    tt.loc[ia, ["요일", "교시"]] = [day_b, int(period_b)]
    new_row = {
        "교사명": teacher_b, "요일": a["요일"], "교시": int(a["교시"]),
        "과목": a["과목"], "학급": a["학급"], "과목군": subject_group(a["과목"])
    }
    tt = pd.concat([tt, pd.DataFrame([new_row])], ignore_index=True)
    st.session_state.timetable = tt.reset_index(drop=True)
    rec = {
        "원본일자": date_a, "교사A": a["교사명"], "요일A": a["요일"], "교시A": int(a["교시"]),
        "학급A": a["학급"], "과목A": a["과목"],
...
def get_teacher_week_view(teacher: str, ref_date: date):
    """달력 기반 교사 1인 주간표 (결강/보강 반영)"""
    weekday = ref_date.weekday()
    monday = ref_date - timedelta(days=weekday)
    week_dates = [monday + timedelta(days=i) for i in range(5)]  # 월~금
    ...
    for d_idx, d in enumerate(DAYS):
        cur_date_str = week_dates[d_idx].strftime("%Y-%m-%d")
        for p in range(1, PERIODS_PER_DAY.get(d, 7) + 1):
            cell = ""
            les = base[(base["요일"] == d) & (base["교시"] == p)]
            if not les.empty:
                cell = f"{les.iloc[0]['과목']} {les.iloc[0]['학급']}"

            # 결강
            abs_m = st.session_state.absences
            if not abs_m.empty:
                if ((abs_m["일자"] == cur_date_str) & (abs_m["교사명"] == teacher) & (abs_m["교시"] == p)).any():
                    cell = f"🚫결강 {cell}" if cell else "🚫결강"

            # 보강 (이 교사가 보강한 경우)
            sub_m = st.session_state.subs
            if not sub_m.empty:
                m = sub_m[(sub_m["일자"] == cur_date_str) & (sub_m["보강교사"] == teacher) & (sub_m["교시"] == p)]
                if not m.empty:
                    cell = f"✅보강 {m.iloc[0]['학급']} {m.iloc[0]['과목']}"

            grid.at[f"{p}교시", d] = cell
...
with st.sidebar:
    ...
    st.subheader("백업 복원")
    backup_up = st.file_uploader("결보강 내역서 백업 (xlsx 권장)", type=["xlsx", "pdf"], key="backup_up")
    if backup_up is not None and st.button("백업 적용", use_container_width=True):
        if backup_up.name.lower().endswith(".xlsx"):
            ok, msg = restore_from_backup_xlsx(backup_up.getvalue())
            if ok:
                st.success(msg)
                st.rerun()
            else:
            ...
            st.warning("PDF는 현재 제한적으로 지원됩니다. 앱에서 내보낸 xlsx 백업을 사용하세요.")
...
# ------------------------------------------------------------------ 5-0. 시간표 조회 (달력 적용)
with tabs[0]:
    view = st.radio("보기 방식", ["교사별 전체 매트릭스", "학급별 매트릭스", "교사 1인 주간표 (달력)"], horizontal=True)
    if view == "교사별 전체 매트릭스":
...
with tabs[1]:
    ...
    if b2.button("시공간 충돌 및 불가능 시간 검사", use_container_width=True):
        ...
        if iss.empty:
            st.success("충돌 없음 — 모든 학급·교사의 중복 및 불가능 시간(0) 배치가 없습니다.")
        else:
            st.error(f"충돌 {len(iss)}건이 발견되었습니다.")
            st.dataframe(iss, use_container_width=True)

# ------------------------------------------------------------------ 5-2. 시간강사 관리
with tabs[2]:
    st.subheader("시간강사 등록 및 가능 시간표 (0=불가능 / 1=가능)")
    st.caption("교사정보와 동일하게 월1~금7 컬럼을 만들어 0/1로 입력하세요. 보강 추천 시 활용됩니다.")
...
with tabs[3]:
    ...
    detail = st.text_input("상세 사유 / 비고 (선택)", placeholder="예: 오전 진료 후 오후 출근")

    if day not in DAYS:
        st.warning("주말은 시간표가 없습니다. 평일을 선택하세요.")
    else:
        ...
        if todays.empty:
            ...
            if st.button("결강 등록", type="primary"):
                if not picked:
                    st.warning("결강할 교시를 하나 이상 선택해주세요.")
                else:
                    try:
                        cid = f"{on_date}-{who}"
                        sel_periods = []
                        ...
                        if new_rows:
                            ...
                            st.warning("선택된 교시에 해당하는 수업이 없습니다.")
                    except Exception as e:
                        st.error(f"결강 등록 중 오류 발생: {e}")
...
with tabs[4]:
    ab = st.session_state.absences
    if ab.empty:
        st.info("먼저 [결강 등록] 탭에서 결강을 등록하세요.")
    else:
        ...
        if not unique_cids:
            ...
            if rows.empty:
                st.error("선택한 결강 건에 해당하는 데이터가 없습니다.")
            else:
                head = rows.iloc[0]
                st.markdown(f"### {head['일자']} ({head['요일']}) · **{head['교사명']}** 교사 · {head['사유']} · 총 {len(rows)}시간")
                ...
                for _, r in rows.iterrows():
                    ...
                    with st.expander(f"{p}교시 · {r['학급']} · {r['과목']}", expanded=True):
                        ...
                        try:
                            ...
                            st.error(f"추천 목록 생성 오류: {e}")
                            cand = pd.DataFrame()

                        if cand.empty:
                            st.error("가능한 공강 교사/시간강사가 없습니다.")
                        else:
                            ...
                            if cc[2].button("배정", key=f"btn_{cid}_{p}", use_container_width=True):
                                try:
                                    ...
                                    st.success(f"{p}교시 보강 교사 배정 완료.")
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"배정 중 오류: {e}")

# ------------------------------------------------------------------ 5-5. 맞교환 & 스마트 변경 추천 (시간강사 구인 체크 포함)
with tabs[5]:
    st.markdown("### 🔄 스마트 시간표 변경 & 맞교환/보강 자동 추천 (연계 교체 지원)")
    st.caption("교사 A의 수업을 이동하고 싶은 **희망 날짜 및 교시**를 입력하면, 직접 1:1 맞교환 · 연계 공강 교환 · 단순 대리 교사를 우선순위 순으로 추천합니다.")
    
    col_a, col_b = st.columns(2)
    
    with col_a:
        st.markdown("#### 1️⃣ 변경 원본 수업 선택 (교사 A)")
        ...
        if sub_a.empty:
            st.warning(f"⚠️ {t_a} 교사는 {date_a_str} ({day_a}요일)에 수업이 없습니다. 다른 날짜를 선택하세요.")
            pick_a = None
        else:
    ...
    with col_b:
        st.markdown("#### 2️⃣ 이동 희망 날짜 & 시간 선택 (Target Date & Time)")
    ...
    is_part_time_purpose = st.checkbox("☑ 이 시간은 시간강사 구인 목적입니다 (시간강사에게 배분할 예정)", value=False)

    st.divider()

    if pick_a:
        st.subheader(f"📌 [{pick_a['일자']}({pick_a['요일']}) {pick_a['교시']}교시 {pick_a['학급']} {pick_a['과목']}] 수업 → [{date_b_str}({day_b}) {p_b_input}교시] 이동 시 추천 대안")
        ...
        with t_swap:
            if df_swap_recs.empty:
                st.info(f"희망 시간대([{day_b}] {p_b_input}교시)에 수업이 있는 다른 교사가 없습니다.")
            else:
                st.caption(f"목표 시간대({day_b} {p_b_input}교시)에 수업이 진행 중인 교사들과의 1:1 맞교환 가능 여부입니다.")
                for idx, row in df_swap_recs.iterrows():
                    b_item = row["b_info"]
                    with st.expander(f"{row['상태']} **{row['교사B']}** 교사 ({row['현재 수업']})", expanded=(idx==0)):
                        st.write(f"- **교사 A ({pick_a['교사명']}) 이동**: {pick_a['일자']}({pick_a['요일']}) {pick_a['교시']}교시  ➔  **{date_b_str}({day_b}) {p_b_input}교시** ({b_item['학급']} 시간대)")
                        st.write(f"- **교사 B ({row['교사B']}) 이동**: {date_b_str}({day_b}) {p_b_input}교시  ➔  **{pick_a['일자']}({pick_a['요일']}) {pick_a['교시']}교시** ({pick_a['학급']} 시간대)")
                        
                        if not row["is_valid"]:
                            st.error("⚠️ **맞교환 불가 원인**")
                            for e in row["errs"]:
                                st.write(f"  • {e}")
                            st.warning("💡 **충돌 해결 안내**")
                            for g in row["guides"]:
                            ...
                            st.success("✅ 시공간 충돌 및 불가능 시간이 없습니다. 맞교환이 가능합니다.")
                            if st.button(f"'{row['교사B']}' 교사와 1:1 맞교환 실행", key=f"exec_swap_{idx}", type="primary"):
                                if do_swap(pick_a, b_item, date_a_str, date_b_str, is_part_time_purpose):
                                    st.success("시간표 맞교환이 성공적으로 실행되었습니다." + (" (시간강사 구인 목적 기록)" if is_part_time_purpose else ""))
                                    st.rerun()

        with t_linked:
            if df_linked_recs.empty:
                st.info("연계 공강 교환이 가능한 교사가 없습니다. (목표 시간 공강 + 원래 시간도 공강 + 가능 여부 + 학급 공백 조건)")
            else:
                st.success("🎯 **연계 교환**이 가장 자연스러운 방법입니다. A가 목표 시간으로 이동하고, B가 A의 원래 수업을 가져갑니다.")
                for idx, row in df_linked_recs.iterrows():
                    with st.expander(f"{row['상태']} **{row['교사B']}** · {row['설명']}", expanded=(idx==0)):
                        st.write(row["설명"])
                        if st.button(f"'{row['교사B']}'와 연계 교환 실행", key=f"exec_linked_{idx}", type="primary"):
                            if do_linked_swap(pick_a, row["교사B"], date_a_str, date_b_str, day_b, p_b_input, is_part_time_purpose):
                                st.success("연계 교환이 완료되었습니다." + (" (시간강사 구인 목적 기록)" if is_part_time_purpose else ""))
                                st.rerun()

        with t_sub:
            if df_sub_recs.empty:
                st.warning(f"목표 시간대({day_b} {p_b_input}교시)에 보강/대리 수업이 가능한 공강 교사가 없습니다.")
            else:
                st.caption(f"목표 시간대({day_b} {p_b_input}교시)에 수업이 없는 교사 중 대리/보강 우선순위 추천 목록입니다.")
                st.dataframe(df_sub_recs[["보강/대리 교사", "우선순위", "담당과목", "누적보강"]], use_container_width=True, hide_index=True)
                
                c_s1, c_s2 = st.columns([3, 1])
                sub_pick_teacher = c_s1.selectbox("대리/보강 지정 교사 선택", df_sub_recs["보강/대리 교사"], key="sel_sub_from_tab")
                if c_s2.button("보강 배정 등록", key="btn_reg_sub", type="primary", use_container_width=True):
                    cid = f"{date_b_str}-{pick_a['교사명']}"
                    pr_val = df_sub_recs.loc[df_sub_recs["보강/대리 교사"] == sub_pick_teacher, "우선순위"].iloc[0]
                    add_substitute(
                        cid, date_b_str, day_b, p_b_input, pick_a["학급"], pick_a["과목"],
                        pick_a["교사명"], sub_pick_teacher, "시간표변경_대리", pr_val, f"{pick_a['일자']} {pick_a['교시']}교시 수업 이동"
                    )
                    st.success(f"[{date_b_str} {p_b_input}교시] 수업 대리 교사로 '{sub_pick_teacher}' 교사가 등록되었습니다.")
...
with tabs[6]:
    ...
    st.download_button("변경 내역서 다운로드 (HTML · 브라우저에서 Ctrl+P → PDF 인쇄)",
    ...
    st.download_button("엑셀 백업 다운로드 (.xlsx)", data=xls,
