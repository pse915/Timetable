...
==========================================================================================
 ...
 2026 서라벌여자중학교용 – 최종 안정화 버전
 (KeyError 수정 + 보강 표시 + 복무 교시 단위 + 맞교환 한눈에 표시)
==========================================================================================
"""
...
DUTY_REQUIRED_COLS = ["교사명", "일자", "교시", "사유", "상세사유", "등록시각"]

def ensure_duty_columns(df: pd.DataFrame) -> pd.DataFrame:
    """duties DataFrame에 필수 컬럼이 항상 존재하도록 보장 (KeyError 방지)"""
    if df is None or not isinstance(df, pd.DataFrame):
        return pd.DataFrame(columns=DUTY_REQUIRED_COLS)
    df = df.copy()
    for col in DUTY_REQUIRED_COLS:
        if col not in df.columns:
            df[col] = "" if col != "교시" else 0
    # 교시 컬럼 숫자화
...
def load_work_data_from_gsheet():
    try:
        ...
        # duties 컬럼 보장 (핵심 수정)
        duties = ensure_duty_columns(duties)
        ...
        return (pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(),
                pd.DataFrame(), pd.DataFrame(columns=DUTY_REQUIRED_COLS))
...
def absent_all_day(teacher: str, on_date: str) -> bool:
    ...
    if not a.empty and "일자" in a.columns:
        if ((a["일자"] == norm_date) & (a["교사명"] == teacher)).any():
            return True
    return has_duty(teacher, norm_date)
...
with tabs[5]:
    ...
    st.info("이 탭은 일반 교사가 결강·보강·맞교환이 반영된 실시간 시간표를 안전하게 확인하고, 변경 시나리오(추천)를 테스트하기 위한 공간입니다.")
...
with tabs[6]:
    ...
    st.caption("선택한 주차에 결강·보강·맞교환이 있는 모든 교사의 주간 시간표를 표시합니다.")
    ...
    st.markdown(f"**📅 주간 범위:** `{week_dates[0].strftime('%Y-%m-%d')}` ~ `{week_dates[4].strftime('%Y-%m-%d')}`")
    st.divider()

    if not changed_teachers:
        st.success("🎉 선택하신 주차에는 변경 내역이 있는 교사가 없습니다.")
        teacher_list = st.session_state.teachers["교사명"].tolist() if not st.session_state.teachers.empty else []
        if teacher_list:
            with st.expander("🔍 특정 교사 주간표 조회"):
        ...
        st.info(f"💡 변경 교사: 총 **{len(changed_teachers)}명** ({', '.join(changed_teachers)})")

        view_mode = st.radio("보기 방식", ["전체 모아보기", "특정 교사만 보기"], horizontal=True, key="week_view_mode")

        if view_mode == "전체 모아보기":
            for t in changed_teachers:
                with st.expander(f"👤 **{t}** 선생님", expanded=True):
...
# ------------------------------------------------------------------ 7. 복무 관리 & 판단 (KeyError 수정 완료)
with tabs[7]:
    st.subheader("📋 복무 일정 관리 & 수업 처리 판단")
    st.caption("왼쪽에서 복무를 등록·선택 → 오른쪽에서 해당 교사의 시간표를 보고 결강/맞교환을 바로 처리합니다.")
    ...
    with left:
        ...
        if st.button("복무 등록", type="primary", key="btn_reg_duty"):
            if duty_teacher and duty_teacher != "없음":
                if not all_day and not selected_periods:
                    ...
                    d = ensure_duty_columns(st.session_state.duties)
                    # 동일 교사+일자 기존 데이터 제거
                    ...
                    st.session_state.duties = ensure_duty_columns(st.session_state.duties)
                    save_work_data_to_gsheet()
                    st.success(f"{duty_teacher} 선생님 복무 등록 완료")
                    st.rerun()
        ...
        if not duties.empty:
            # 안전한 정렬 (존재하는 컬럼만 사용)
            sort_cols = [c for c in ["일자", "교사명", "교시"] if c in duties.columns]
            display_df = duties.sort_values(sort_cols).reset_index(drop=True) if sort_cols else duties.reset_index(drop=True)

            display_df_show = display_df.copy()
            if "교시" in display_df_show.columns:
                display_df_show["교시표시"] = display_df_show["교시"].apply(lambda x: "전체" if safe_int(x) == 0 else f"{safe_int(x)}교시")
            else:
                display_df_show["교시표시"] = ""

            show_cols = [c for c in ["교사명", "일자", "교시표시", "사유", "상세사유"] if c in display_df_show.columns]
            st.dataframe(display_df_show[show_cols], use_container_width=True, height=250)
        ...
        if not selected_duty:
            ...
            if day_kr not in DAYS:
                ...
                if target_lessons.empty:
                    ...
                    st.caption(f"총 {len(target_lessons)}시간")
                        ...
                        with st.expander(f"{p_val}교시 · {cls_val} · {sub_val}  {status}", expanded=is_duty_period and not (is_subbed or is_absent or is_swapped)):
                            ...
                            if possible_swaps:
                                ...
                                for i, sw in enumerate(possible_swaps[:4]):
                                    st.write(f"• **{sw.get('유형')}** | {sw.get('교사B')} | {sw.get('현재 수업')}")
                            ...
                            if act1.button("결강으로 등록", key=f"duty_abs_{p_val}"):
                                ...
                                save_work_data_to_gsheet()
                                st.success(f"{p_val}교시 결강 등록 완료")
                                st.rerun()
